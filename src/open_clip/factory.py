...
from .transform import image_transform_v2, AugmentationCfg, PreprocessCfg, merge_preprocess_dict, merge_preprocess_kwargs, image_transform
...
def create_model(
    ...
    #force_siglip_preprocess = model_kwargs.pop("force_siglip_preprocess", False)
...
def create_loss(args):
    ...
    if args.use_pseudo_labels:
        assert args.distill, "Pseudo-labeling requires a teacher model"
        assert not args.horovod, "Horovod not currently supported for SigLip"
        return FilteredSigLipLoss(
            rank=args.rank,
            world_size=args.world_size,
            pseudo_txt_txt_th=args.pseudo_txt_txt_th,
...
def create_model_and_transforms(
    ...
    if force_siglip_preprocess:
        image_preprocess = { # Default ViT-B-16-SigLIP/webli config
            "mean": [0.5, 0.5, 0.5],
            "std": [0.5, 0.5, 0.5],
            "image_size": 224,
        ...
        preprocess_train = image_transform(
            mean=image_preprocess["mean"],
            std=image_preprocess["std"],
            interpolation=image_preprocess["interpolation"],
            image_size=image_preprocess["image_size"],
            resize_mode=image_preprocess["resize_mode"],
            is_train=True,
        )
