...
def train_one_epoch(model, data, loss, epoch, optimizer, scaler, scheduler, dist_model, args, tb_writer=None):
    ...
    for i, batch in enumerate(dataloader):
        ...
        if args.num_captions > 1:
            b, num_caps, d = texts.size()
            texts = texts.reshape(b*num_caps, d)

            if args.distill:
                b, num_caps, d = texts_distill.size()
                texts_distill = texts_distill.reshape(b*num_caps, d)
        ...
        if args.accum_freq == 1:
            with autocast():
                ...
                if args.npc_loss or args.xac_loss:
                    ...
                    if args.distill:
                        with torch.no_grad():
                            dist_model_out = dist_model(images_distill, texts_distill)
